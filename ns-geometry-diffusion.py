...
# Create mesh
channel = Rectangle(Point(0, 0), Point(2.0, 1.0))
...
# Define boundaries
inflow  = 'near(x[1], 1.0) && x[0] < 0.2'
outflow = 'near(x[1], 1.0) && x[0] > 1.8'
walls   = 'on_boundary && (!(near(x[1], 1.0) && x[0] > 1.8)) && (!(near(x[1], 1.0) && x[0] < 0.2))'

# Define inflow profile
inflow_profile = ('0', '-4.0*1.5*x[0]*(0.2 - x[0]) / pow(0.2, 2)')
...
bcp = [bcp_outflow]
bc = DirichletBC(C, Constant(1.0), inflow)
...
xdmffile_u = XDMFFile('ns/velocity.xdmf')
xdmffile_p = XDMFFile('ns/pressure.xdmf')
vtkfile_c = File('ns/c.pvd')
